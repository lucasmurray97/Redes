...
class RoundRobin:
    ...
    def calc_posible_routes(self, dest_ip, dest_port):
        ...
        for i in self.routes_dict.keys():
            ...
            route = self.routes_dict[i]["bgp_route"]
            # port_range = self.routes_dict[i]["port_range"]
            if dest_address_bin[:ip_range] == net_ip[:ip_range] and dest_port == self.routes_dict[i]["route_dest"]:
    ...
    def get_route(self, dest_ip, dest_port):
        ...
        if posible_routes_idx != None: 
            ...
            if (dest_ip, dest_port) not in self.cache.keys():
                ...
                return (self.routes_dict[key]["next_hop_ip"], self.routes_dict[key]["next_hop_port"]), self.routes_dict[key]["mtu"]
            # Si ya se han enviado mensajes a ese destino
                ...
                while True:
                    if next(posible_routes_idx) == key:
                        ...
                        # Retornamos esa ruta
                        return (self.routes_dict[next_key]["next_hop_ip"], self.routes_dict[next_key]["next_hop_port"]), self.routes_dict[key]["mtu"]
